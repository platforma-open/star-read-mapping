 ...
 */
export const parseColId = (str: string) =>
  JSON.parse(atob(str)) as { columnIndex: number; id: PTableColumnId };
...
function columns2rows(
  ...
  for (var iRow = 0; iRow < nRows; ++iRow) {
    ...
    row["id"] = JSON.stringify(axes, (_, v) =>
 ...
 */
const sortChanged = (
  oldSort: SortModelItem[] | undefined,
  newSort: SortModelItem[] | undefined
) => {
 ...
 */
export async function pFrameGridOptions(
  ...
  const columnDefs = [];
  for (var columnIndex = 0; columnIndex < ptShape.columns; ++columnIndex) {
...
export async function pFrameFilters(
  pfDriver: PFrameDriver,
  pt: PTableHandle,
  axes: AxisId[]
) {
  const axesSet = new Set<AxisId | any>(axes);
  const specs = await pfDriver.getSpec(pt);

  for (const spec of specs) {
    if (axesSet.has(spec.id)) {
    }
  }
}
